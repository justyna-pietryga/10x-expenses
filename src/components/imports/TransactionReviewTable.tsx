...
function getDraftState(
  ...
  if (typeof draft === "string") {
    return {
      category_id: draft === "" ? null : draft,
      inclusion_status: (transaction.inclusion_status as TransactionInclusion | undefined) ?? "included",
    };
  }
...
export function buildInitialReviewRuleDraft(
  ...
  return {
    category_id: draftState.category_id,
...
export function buildDirtyCategoryUpdates(
  ...
  return transactions.flatMap((transaction) => {
    const draft = getDraftState(transaction, drafts, transaction.id);
    const hasDraft = drafts[transaction.id] !== undefined;

    if (!hasDraft) {
      return [];
    }

    if (transaction.category_id === draft.category_id && transaction.inclusion_status === draft.inclusion_status) {
      return [];
    }
    ...
    return [
      {
        ...
        inclusion_status: draft.inclusion_status,
        transaction_id: transaction.id,
      },
    ];
  });
...
export function buildBulkSaveFeedback(
  drafts: Partial<Record<string, TransactionReviewDraft>>,
  result: ImportCategorySaveResult,
) {
  ...
  result.updated.forEach(({ id }) => {
    ...
    successById[id] = "Review change saved.";
  });
...
export function TransactionReviewTable({
  ...
  const [showExcludedRows, setShowExcludedRows] = useState(initialShowExcluded);
  ...
  const includedTransactions = transactions.filter(
    (transaction) => getDraftState(transaction, drafts, transaction.id).inclusion_status === "included",
  );
  const excludedTransactions = transactions.filter(
    (transaction) => getDraftState(transaction, drafts, transaction.id).inclusion_status === "excluded",
  );
  ...
  function updateDraft(transaction: ImportedTransactionReviewRow, update: Partial<TransactionReviewDraft>) {
    setDrafts((current) => ({
      ...current,
      [transaction.id]: {
        ...getDraftState(transaction, current, transaction.id),
        ...update,
      },
    }));
  ...
  return (
    <section className="rounded-[28px] border border-white/12 bg-white/8 p-6 shadow-[0_20px_70px_rgba(2,6,23,0.35)] backdrop-blur-xl">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold text-white">
            Adjust budget inclusion and categories without editing source values.
          </h2>
        </div>
        <div className="rounded-full border border-white/12 bg-slate-950/30 px-4 py-2 text-sm text-slate-200">
          {includedTransactions.length} included row{includedTransactions.length === 1 ? "" : "s"} in the default view
        </div>
      </div>
      ...
      {dirtyCount > 0 && (
        <div className="mt-4 flex flex-col gap-3 rounded-3xl border border-cyan-200/15 bg-slate-950/35 p-4 lg:flex-row lg:items-center lg:justify-between">
          <p className="text-sm font-medium text-cyan-100">
            {dirtyCount} unsaved review change{dirtyCount === 1 ? "" : "s"}
          </p>
      ...
      {excludedTransactions.length > 0 && (
        <div className="mt-6 rounded-3xl border border-white/12 bg-slate-950/30 p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-sm font-medium text-white">
                {excludedTransactions.length} excluded row{excludedTransactions.length === 1 ? "" : "s"} kept out of
                budget calculations
              </p>
              <p className="mt-1 text-sm text-slate-300">
                Excluded rows stay preserved for audit history and can be restored intentionally.
              </p>
            </div>
            <Button
              type="button"
              ...
              onClick={() => {
                setShowExcludedRows((current) => !current);
              }}
            >
              {showExcludedRows ? "Hide excluded rows" : "Reveal excluded rows"}
            </Button>
          </div>
        </div>
      )}
            ...
            {includedTransactions.map((transaction) => {
              ...
              const isDirty =
                ...
                (transaction.category_id !== draftState.category_id ||
                  transaction.inclusion_status !== draftState.inclusion_status);
              ...
              return (
                <tr key={transaction.id} className="rounded-3xl bg-slate-950/28">
                  ...
                  <td className="px-4 py-4 align-top">
                    <select
                      value={getCategorySelectValue(draftState.category_id)}
                      onChange={(event) => {
                    ...
                    {currentRuleDraft ? (
                      ...
                      <div className="space-y-2">
                        ...
                        <Button
                          type="button"
                          variant="ghost"
                          className="rounded-2xl border border-amber-200/25 text-amber-100 hover:bg-amber-200/10"
                          onClick={() => {
                            updateDraft(transaction, {
                              inclusion_status: "excluded",
                            });
                            setRuleDraftById((current) => ({ ...current, [transaction.id]: undefined }));
                          }}
                        >
                          Exclude from budget
                        </Button>
      ...
      {showExcludedRows && excludedTransactions.length > 0 && (
              ...
              {excludedTransactions.map((transaction) => {
                ...
                return (
                  <tr key={transaction.id} className="rounded-3xl bg-slate-950/28">
                    ...
                    <td className="px-4 py-4 align-top text-slate-300">Excluded from budget totals</td>
                    <td className="rounded-r-3xl px-4 py-4 text-right align-top">
                      <Button
                        type="button"
                        variant="ghost"
                        className="rounded-2xl border border-emerald-200/25 text-emerald-100 hover:bg-emerald-200/10"
                        onClick={() => {
                          updateDraft(transaction, {
