...
export function IncompleteReviewNotice({ incompleteReviewSpend, warningBatches }: Props) {
  ...
  return (
    <section className="rounded-[28px] border border-amber-300/25 bg-amber-300/10 p-6 text-amber-50 shadow-[0_18px_60px_rgba(120,53,15,0.18)]">
      ...
      <h2 className="mt-2 text-2xl font-semibold">Incomplete imported expense review stays separate.</h2>
      <p className="mt-3 text-sm leading-6 text-amber-50/85">
        {formatAmount(incompleteReviewSpend)} of imported expense spend is still waiting for review, so it is excluded
        from trusted category totals. Rows you already excluded stay out of this warning and appear in the excluded
        transactions panel instead. Return to{" "}
