...
test("import review does not allow completion while category drafts are unsaved [risk #3]", async ({
  ...
  try {
    ...
    await expect(page.getByText("1 unsaved review change")).toBeVisible();
    await expect(page.getByText("Save or discard review changes before marking this review complete.")).toBeVisible();
    await expect(completeReviewButton).toBeDisabled();

    await page.getByRole("button", { name: "Save all changes" }).click();

    await expect(page.getByText("Category changes saved.")).toBeVisible();
    await expect(page.getByText("1 unsaved review change")).not.toBeVisible();
    await expect(
      page.getByText("Save or discard review changes before marking this review complete."),
    ).not.toBeVisible();
    await expect(completeReviewButton).toBeEnabled();
  } finally {
    if (categoryId) {
